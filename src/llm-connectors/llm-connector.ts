...
export abstract class LLMConnector {
    ...
    **Instructions:**
    ...
    2. Reflect on the captcha type and goal:
       - If it’s a “select all squares with X” captcha, what squares remain unchecked based on the image and history? Estimate their positions (e.g., top-left corner might be around x: 10, y: 10).
    ...
    4. Decide the next action(s) to advance the captcha solution. Output a single JSON object for one action or an array of up to four JSON objects for multiple actions. Ask yourself:
       ...
       - Are multiple clicks needed (e.g., for a grid captcha)? If so, prioritize the most obvious targets first.
